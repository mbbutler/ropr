...
impl RVInstruction {
	...
	pub fn is_gadget_tail(&self, rop: bool, sys: bool, jop: bool, noisy: bool) -> bool {
		if self.is_invalid() {
			return false;
		}
	...
	pub fn is_rop_gadget_head(&self, noisy: bool) -> bool {
		if self.is_invalid() {
			return false;
		}
		match self.flow_control() {
	...
	pub fn is_stack_pivot_head(&self) -> bool {
		self.modifies_reg(rv_ireg::sp as u8)
	}

	pub fn is_stack_pivot_tail(&self) -> bool {
		self.is_ret()
	}

	pub fn is_base_pivot_head(&self) -> bool {
